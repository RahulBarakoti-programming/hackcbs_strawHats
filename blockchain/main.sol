...
contract DataMarketplace {
    ...
    struct Request {
        ...
        address buyerPublicKey; // Assuming buyerPublicKey is an address, this remains as a placeholder
    }
    ...
    modifier onlyDataOwner(uint256 _dataId, address _ownerAddress) {
        require(
            dataRegistry[_dataId].ownerAddress == _ownerAddress,
            "Not the data owner"
        );
        _;
    }

    modifier validDataInput(
        string memory _ipfs,
        string memory _name,
        uint256 _price
    ) {
    ...
    function addData(
        ...
        string memory _pass
    ) public validDataInput(_ipfs, _name, _price) returns (uint256) {
    ...
    function acceptRequest(
        ...
        address _buyerPublicKey
    ) public onlyDataOwner(_dataId, _ownerAddress) returns (bool) {
        ...
        // Logging the buyer and buyerPublicKey details
        Request memory newRequest = Request({
            id: _dataId,
            buyerAddress: _buyerAddress,
            buyerPublicKey: _buyerPublicKey
        });
        userRequests[_ownerAddress].push(newRequest);
    ...
    function deactivateData(
        uint256 _dataId,
        address _ownerAddress
    ) public onlyDataOwner(_dataId, _ownerAddress) {
